...
<div id="tribe-events-event-<?php echo $event_id ?>" class="<?php tribe_events_event_classes() ?>" data-tribejson='<?php echo tribe_events_template_data( $post ); ?>'>
	<h3 class="tribe-events-month-event-title entry-title summary"><a href="<?php tribe_event_link( $post ); ?>" class="url"><?php if(get_field('calendar_display_name', $post->ID)) { the_field('calendar_display_name', $post->ID); } else { the_title(); } ?></a></h3>
