...
<?php 
	...
	$allowed_roles = array('administrator', 'manager', 'lessee');
?>

<?php if (in_array($user_role, $allowed_roles)) : ?>

<div class="profile_table-full">
<h2><?php _e( 'My Lease Payments', 'woocommerce-subscriptions' ); ?></h2>
<?php $subscriptions = WC_Subscriptions_Manager::get_users_subscriptions( get_current_user_id() ); ?>
<?php $activeSubscriptions = false; ?>
<?php 
...
?>
	<?php if ( ! empty( $subscriptions ) ) : ?>
		<?php if ($activeSubscriptions === true) : ?>
		...
		<table class="shop_table my_account_subscriptions my_account_orders">
			...
			<thead>
				<tr>
					...
					<th class="subscription-title"><span class="nobr"><?php _e( 'Lease', 'woocommerce-subscriptions' ); ?></span></th>
		...
		<?php endif; // end if to check if there are active subscriptions ?>
	
	<?php endif; ?>
<?php 
	if ($activeSubscriptions === true) {
		echo 'Please cancel your automatic payments to make a one-time payment.';
	} else {
		echo '<p><a href="/product/automatic-payments/">Activate your automatic payments</a></p>';
		echo '<p><a href="/product/manual-payments/">Make a one-time payment</a></p>';
	}
?>
</div>

<?php else : ?>
<?php echo do_shortcode('[gravityform id="8" name="Sign up for online payments" title="true" description="false"]'); ?>
